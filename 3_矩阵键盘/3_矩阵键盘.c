...
sbit du = P2^6;
sbit we = P2^7;
uchar key = 19;//uchar key = 20;//数组中并没有第20号，19号就是熄灭。
...
void KeyValue()
{
	...
	//逻辑错误，只要按下按键行列代码就会发生变化，不用再创建一个if语句；不过我们先不动它，看看是不是决定性错误。
			P3 = 0XF0;
			if(P3!= 0XF0)
			{
				delay(20);
			...
			if(P3!= 0Xff)
			{
				...
				if(P3!= 0XfF)
				{
						...
						while (P3 != 0xff)//while = (P3 != 0xfe);//应该是打错了
				}
...
{/*
			while(1)
	{
			...
			du = 1;
			du = 0;
			我并不是很清楚为什么位选和段选需要放在while外，不过目前确实没有看到把位选和段选放在while中的，那就把这个当成硬性规定了。
	}		*/
	du = 0;
	we = 0;
	delay(5);
